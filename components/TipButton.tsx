...
const CONTRACT_ADDRESS = "0x0178d9E3c16179b015e21FB58A3A6Bcc843da5Ca"
...
export default function TipButton({ postId, authorName, onTipSuccess }: TipButtonProps) {
  ...
  const handleTip = async () => {
    ...
    try {
      ...
      console.log(`Tipped ${tipAmount} cBTC to ${authorName} for post ${postId}`)
      ...
      {showTipModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">Amount (CELO)</label>
              <input
                type="number"
                step="0.001"
                min="0.001"
            ...
            <div className="flex gap-3">
              ...
              >
                {isLoading ? "Processing..." : `Tip ${tipAmount} CELO`}
              </button>
            </div>
          </div>
        </div>
